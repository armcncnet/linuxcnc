...
func (launch *Launch) Start(machine string) {
	...
	if write == nil {
		if machine != "" {
			cmd := exec.Command("systemctl", "restart", "armcnc_linuxcnc.service armcnc_launch.service")
			cmd.Output()
		} else {
			cmd := exec.Command("systemctl", "stop", "armcnc_linuxcnc.service armcnc_launch.service")
			cmd.Output()
		}
	} else {
		cmd := exec.Command("systemctl", "stop", "armcnc_linuxcnc.service armcnc_launch.service")
